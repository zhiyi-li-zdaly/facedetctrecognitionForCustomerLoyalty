...
# Version 32: Deal with noisy category, duplicate category.
...
if (v_cap.isOpened()): 
    ...
    out_writter = cv2.VideoWriter('samples/video_33.avi',cv2.VideoWriter_fourcc('M','J','P','G'), 25, (width, height))
...
local_file_name = "./samples/timeRecordCountingInformationPer10mins_sample1_v33.txt"
...
path = 'data_v1/test_images33'
face_path = 'data_v1/test_face_images33'
...
while(v_cap.isOpened()):
    ...
    num += 1
    if num % 10 == 1: # Every 15 frames, perform detection. 
        print ("num: ", num)
        ...
        if boxes is not None:	# Handle multiple faces
            for i, (face, box, point) in enumerate(zip(faces, boxes, points)):
                ...
                # This part is load faces from file directory every 6 hours
                delta_s = (cur_time - start_time).total_seconds()
                delta_m = int(delta_s / 60)
                delta_h = int(delta_s / 60 / 60)
                print ("delta_s: ", delta_s)
                print ("delta_m: ", delta_m)
                print ("delta h: ", delta_h)
                
                #########################################
                ### Reload category/person from the file.
                if delta_m % 5 == 1:
                    aligned = []
                    names = []
                    dir = os.listdir(path)
